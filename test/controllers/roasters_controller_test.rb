...
class RoastersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create roaster" do
    assert_difference('Roaster.count') do
      post roasters_url, params: { roaster: { location: @roaster.location, name: @roaster.name } }
    end
  ...
  test "should update roaster" do
    patch roaster_url(@roaster), params: { roaster: { location: @roaster.location, name: @roaster.name } }
